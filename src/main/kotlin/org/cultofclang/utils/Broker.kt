...
data class SimpleRecipe(
    ...
    fun stringify() = Yaml.default.encodeToString(serializer(), this)
...
data class MarketBook(val values: Map<Material, Double>, val recipes: List<SimpleRecipe>) {
    override fun toString(): String {
        return Yaml.default.encodeToString(MarketBook.serializer(), this)
    }
...
fun List<ItemStack>.sum(): Mats {
    return this.groupBy { it.type }.map { it.key to it.value.sumOf { a -> a.amount } }.toMap()
}
